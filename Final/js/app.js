...
function leerDatosCatalogo(catalogo) {
    const infocatalogo = {
        ...
        id: catalogo.querySelector('a').getAttribute('date-id')
    }
...
function eliminarCatalogoLocalStorage(catalogo) {
    ...
    catalogossLS = obtenerCatalogosLocalStorage();

    catalogossLS.forEach(function(catalogossLS, index){
        if(catalogossLS.id === catalogo) {
            catalogossLS.splice(index, 1)
        }
    });
...
function buscador_interno(){
    filtar = inputSearch.value.toUpperCase();
